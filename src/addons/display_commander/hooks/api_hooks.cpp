...
EXECUTION_STATE WINAPI SetThreadExecutionState_Detour(EXECUTION_STATE esFlags) {
  // Get screensaver mode setting
  int screensaver_mode = s_screensaver_mode.load();

  // If screensaver mode is 1 (disable in background) or 2 (disabled), ignore
  // the call
  if (screensaver_mode == 1 || screensaver_mode == 2) {
    // Return 0 to indicate the call was blocked
    return 0;
  }

  // For mode 0 (default), call the original function
