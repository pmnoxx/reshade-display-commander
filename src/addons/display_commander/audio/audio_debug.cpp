...
void DebugAudioSessions() {
    ...
    do {
        ...
        oss << "Found " << count << " audio sessions";
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (FAILED(session_enumerator->GetSession(i, &session_control)) || session_control == nullptr) {
                std::ostringstream oss;
                oss << "Failed to get audio session " << i;
...
void DebugAudioSessionForProcess(DWORD process_id) {
    ...
    do {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            if (SUCCEEDED(session_control->QueryInterface(&session_control2)) && session_control2 != nullptr) {
                ...
                if (pid == process_id) {
                    found_process = true;
                    std::ostringstream oss;
                    oss << "Found audio session for process " << process_id << " (session " << i << ")";
                    LogInfo(oss.str().c_str());
                    LogAudioSessionInfo(session_control, i);
                }
        ...
        if (!found_process) {
            std::ostringstream oss;
            oss << "No audio session found for process " << process_id;
            LogWarn(oss.str().c_str());
        }
    } while (false);
...
void LogAudioSessionInfo(IAudioSessionControl *session_control, int session_index) {
    ...
    if (SUCCEEDED(hr)) {
        ...
        std::ostringstream oss;
        oss << "  Session " << session_index << ": State = " << state_str;
        LogInfo(oss.str().c_str());
    }
    ...
    if (SUCCEEDED(hr) && display_name != nullptr) {
        ...
        if (size_needed > 0) {
            ...
            std::ostringstream oss;
            oss << "  Session " << session_index << ": Display Name = " << buffer.data();
            LogInfo(oss.str().c_str());
        }
    ...
    if (SUCCEEDED(hr) && icon_path != nullptr) {
        ...
        if (size_needed > 0) {
            ...
            std::ostringstream oss;
            oss << "  Session " << session_index << ": Icon Path = " << buffer.data();
            LogInfo(oss.str().c_str());
        }
    ...
    if (SUCCEEDED(session_control->QueryInterface(&session_control2)) && session_control2 != nullptr) {
        ...
        if (SUCCEEDED(hr)) {
            std::ostringstream oss;
            oss << "  Session " << session_index << ": Process ID = " << pid;
            LogInfo(oss.str().c_str());
        }
        ...
        if (SUCCEEDED(hr) && session_id != nullptr) {
            ...
            if (size_needed > 0) {
                ...
                std::ostringstream oss;
                oss << "  Session " << session_index << ": Session ID = " << buffer.data();
                LogInfo(oss.str().c_str());
            }
    ...
    if (SUCCEEDED(session_control->QueryInterface(&simple_volume)) && simple_volume != nullptr) {
        ...
        if (SUCCEEDED(hr)) {
            std::ostringstream oss;
            oss << "  Session " << session_index << ": Master Volume = " << std::fixed << std::setprecision(2)
                << volume;
            LogInfo(oss.str().c_str());
        }
        ...
        if (SUCCEEDED(hr)) {
            std::ostringstream oss;
            oss << "  Session " << session_index << ": Muted = " << (muted ? "Yes" : "No");
            LogInfo(oss.str().c_str());
        }
