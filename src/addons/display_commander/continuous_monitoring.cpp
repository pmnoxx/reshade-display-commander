...
void every1s_checks() {
  ...
  if (hwnd != nullptr && IsWindow(hwnd)) {
    // BACKGROUND DETECTION: Check if the app is in background using original
    // GetForegroundWindow
    ...
    bool background_changed =
        (current_background != g_app_in_background.load());
    ...
    // Apply window changes - the function will automatically determine what
    // needs to be changed
    ApplyWindowChange(hwnd, "continuous_monitoring_auto_fix");

    // BACKGROUND WINDOW MANAGEMENT: Update background window if feature is
    // enabled
    static int background_check_counter = 0;
    if (++background_check_counter % 10 == 0) { // Log every 10 seconds
      std::ostringstream oss;
      oss << "Continuous monitoring: Background feature check - "
             "s_background_feature_enabled = "
          << (s_background_feature_enabled.load() ? "true" : "false")
          << ", has_background_window = "
          << g_backgroundWindowManager.HasBackgroundWindow();
      LogInfo(oss.str().c_str());
    }

    // FOCUS LOSS DETECTION: Close background window when main window loses
    // focus
    ...
    if (s_background_feature_enabled.load()) {
      ...
      if (foreground_window == hwnd) {
        LogInfo("Continuous monitoring: Calling UpdateBackgroundWindow for "
        ...
        if (background_check_counter % 10 == 0) { // Log occasionally
          LogInfo("Continuous monitoring: Skipping background window update - "
                  "main window not focused");
        }
      }
    } else {
      if (background_check_counter % 10 == 0) { // Log occasionally
        LogInfo("Continuous monitoring: Background feature disabled "
                "(s_background_feature_enabled = false)");
      }
    }
  }

  // BACKGROUND: Composition state logging and periodic device/colorspace
  // refresh NOTE: This functionality has been moved to OnPresentUpdateAfter to
  // avoid accessing g_last_swapchain_ptr from the continuous monitoring thread
  ...
  // Aggregate FPS/frametime metrics and publish shared text once per second
  {
    ...
    // Handle reset: clear samples efficiently by advancing head and zeroing
    // text
    ...
    const uint32_t count = (head > static_cast<uint32_t>(kPerfRingCapacity))
                               ? static_cast<uint32_t>(kPerfRingCapacity)
                               : head;
    ...
    if (!fps_values.empty()) {
      ...
      fps_display =
          (total_seconds > 0.0)
              ? static_cast<float>(static_cast<double>(n) / total_seconds)
              : 0.0f;
      ...
      frame_time_ms =
          (n % 2 == 1)
              ? ft_for_median[n / 2]
              : 0.5f * (ft_for_median[n / 2 - 1] + ft_for_median[n / 2]);

      // Compute lows and top frame times using frame time distribution (more
      // robust)
      ...
      const size_t count_1 =
          (std::max<size_t>)(static_cast<size_t>(static_cast<double>(m) * 0.01),
                             size_t(1));
      const size_t count_01 =
          (std::max<size_t>)(static_cast<size_t>(static_cast<double>(m) *
                                                 0.001),
                             size_t(1));
      ...
      one_percent_low =
          (avg_ft_1 > 0.0) ? static_cast<float>(1000.0 / avg_ft_1) : 0.0f;
      ...
      point_one_percent_low =
          (avg_ft_01 > 0.0) ? static_cast<float>(1000.0 / avg_ft_01) : 0.0f;

      // Percentile frame times (top 1%/0.1% = 99th/99.9th percentile)
      const size_t idx_p99 =
          (m > 1)
              ? (std::min<size_t>)(m - 1, static_cast<size_t>(std::ceil(
                                              static_cast<double>(m) * 0.99)) -
                                              1)
              : 0;
      const size_t idx_p999 =
          (m > 1)
              ? (std::min<size_t>)(m - 1, static_cast<size_t>(std::ceil(
                                              static_cast<double>(m) * 0.999)) -
                                              1)
              : 0;
    ...
    fps_oss << "FPS: " << std::fixed << std::setprecision(1) << fps_display
            ...
            << ")"
            << "   Top FT: P99 " << std::setprecision(1) << p99_frame_time_ms
            << " ms"
            << ", P99.9 " << std::setprecision(1) << p999_frame_time_ms << " ms"
            << " since reset";
    g_perf_text_shared.store(
        std::make_shared<const std::string>(fps_oss.str()));
  }
      ...
      if (!g_dlssfg_detected.load()) {
        if (g_dlssfgVersionDetector.RefreshVersion()) {
          if (g_dlssfgVersionDetector.IsAvailable()) {
            ...
            g_dlls_g_version.store(std::make_shared<const std::string>(
                version.getFormattedVersion()));
          }
        }

        // DLSS Preset Detection: Check every 5 seconds for runtime-loaded DLSS
        // presets
        if (!g_dlss_preset_detected.load()) {
          if (g_dlssPresetDetector.RefreshPreset()) {
            if (g_dlssPresetDetector.IsAvailable()) {
              ...
              LogInfo(
                  "DLSS Preset detected at runtime - Preset: %s, Quality: %s",
              ...
              g_dlss_preset_name.store(std::make_shared<const std::string>(
                  preset.getFormattedPreset()));
              g_dlss_quality_mode.store(std::make_shared<const std::string>(
                  preset.getFormattedQualityMode()));
            }
          }
        }
      }
...
// Screensaver management function - called every 16ms
void every16ms_screensaver_checks() {
  static DWORD last_check_time = 0;
  static EXECUTION_STATE last_execution_state = 0;

  DWORD current_time = GetTickCount();
  if (current_time - last_check_time < 16) {
  ...
  if (screensaver_mode == 0) {
    // No Change mode - don't interfere
    return;
  }

  // Determine what execution state we want to set
  EXECUTION_STATE desired_state = 0;
  bool should_prevent_screensaver = false;

  if (screensaver_mode == 2) {
    // Always Disable mode
    should_prevent_screensaver = true;
  } else if (screensaver_mode == 1) {
    // Disable in Foreground mode - check if game window is in foreground
    HWND hwnd = g_last_swapchain_hwnd.load();
    if (hwnd != nullptr && IsWindow(hwnd)) {
      HWND foreground_window =
          renodx::hooks::GetForegroundWindow_Original
              ? renodx::hooks::GetForegroundWindow_Original()
              : GetForegroundWindow();
      should_prevent_screensaver = (foreground_window == hwnd);
    }
  }

  if (should_prevent_screensaver) {
    desired_state = ES_CONTINUOUS | ES_DISPLAY_REQUIRED;
  } else {
    desired_state = ES_CONTINUOUS;
  }

  // Only call SetThreadExecutionState if the desired state has changed
  if (desired_state != last_execution_state) {
    if (renodx::hooks::SetThreadExecutionState_Original) {
      EXECUTION_STATE result =
          renodx::hooks::SetThreadExecutionState_Original(desired_state);
      last_execution_state = desired_state;

      if (should_prevent_screensaver) {
        LogInfo("Screensaver prevention enabled - execution state: 0x%08X, "
                "result: 0x%08X",
                desired_state, result);
      } else {
        LogInfo("Screensaver prevention disabled - execution state: 0x%08X, "
                "result: 0x%08X",
                desired_state, result);
      }
    } else {
      // Fallback to direct API call if hook is not available
      EXECUTION_STATE result = SetThreadExecutionState(desired_state);
      last_execution_state = desired_state;

      if (should_prevent_screensaver) {
        LogInfo("Screensaver prevention enabled (direct) - execution state: "
                "0x%08X, result: 0x%08X",
                desired_state, result);
      } else {
        LogInfo("Screensaver prevention disabled (direct) - execution state: "
                "0x%08X, result: 0x%08X",
                desired_state, result);
      }
    }
...
void ContinuousMonitoringThread() {
  ...
  const LONGLONG FPS_60_INTERVAL_NS =
      utils::SEC_TO_NS / 60; // ~16.67ms for 60 FPS
  ...
  while (g_monitoring_thread_running.load()) {
    // Periodic display cache refresh off the UI thread
    {
      ...
      if (now_ns - last_cache_refresh_ns >= 2 * utils::SEC_TO_NS) {
        ...
        auto next =
            std::make_shared<const std::vector<std::string>>(std::move(labels));
    ...
    if (now_ns - last_60fps_update_ns >= FPS_60_INTERVAL_NS) {
      ...
      // Screensaver management at 60 FPS (~16ms)
      every16ms_screensaver_checks();
    }
