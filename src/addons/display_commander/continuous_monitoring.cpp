...
static EXECUTION_STATE g_current_execution_state = 0;

void UpdateScreensaverState() {
  int screensaver_mode = s_screensaver_mode.load();

  // Determine if we should disable screensaver
  bool should_disable_screensaver = false;

  if (screensaver_mode == 2) {
    // Mode 2: Always disabled
    should_disable_screensaver = true;
  } else if (screensaver_mode == 1) {
    // Mode 1: Disable in background
    bool is_background = g_app_in_background.load();
    should_disable_screensaver = is_background;
  }
  // Mode 0: Default behavior (don't disable)

  // Calculate desired execution state
  EXECUTION_STATE desired_state = ES_CONTINUOUS;
  if (should_disable_screensaver) {
    desired_state = ES_CONTINUOUS | ES_DISPLAY_REQUIRED;
  }

  // Only call SetThreadExecutionState if the state has changed
  if (desired_state != g_current_execution_state) {
    g_current_execution_state = desired_state;

    // Call the original SetThreadExecutionState function
    if (renodx::hooks::SetThreadExecutionState_Original) {
      renodx::hooks::SetThreadExecutionState_Original(desired_state);
      LogInfo("Screensaver state updated: mode=%d, background=%s, state=0x%x",
              screensaver_mode, g_app_in_background.load() ? "true" : "false",
              desired_state);
    }
  }
}
...
void every1s_checks() {
  ...
  // Update screensaver state based on current settings and background state
