...
std::atomic<bool> s_suppress_binding_in_background{
    false}; // Keep binding by default to avoid state issues
...
bool ShouldBackgroundSuppressOperation() {
    ...
    return s_no_render_in_background.load() &&
           g_app_in_background.load(std::memory_order_acquire);
}

// Power saving for compute shader dispatches
bool OnDispatch(reshade::api::command_list *cmd_list, uint32_t group_count_x,
    ...
    if (s_suppress_compute_in_background.load() &&
...
bool OnDispatchMesh(reshade::api::command_list *cmd_list,
...
bool OnDispatchRays(reshade::api::command_list *cmd_list,
                    reshade::api::resource raygen, uint64_t raygen_offset,
                    uint64_t raygen_size, reshade::api::resource miss,
                    uint64_t miss_offset, uint64_t miss_size,
                    uint64_t miss_stride, reshade::api::resource hit_group,
                    uint64_t hit_group_offset, uint64_t hit_group_size,
                    uint64_t hit_group_stride, reshade::api::resource callable,
                    uint64_t callable_offset, uint64_t callable_size,
                    uint64_t callable_stride, uint32_t width, uint32_t height,
                    uint32_t depth) {
    // Increment event counter
...
bool OnCopyResource(reshade::api::command_list *cmd_list,
    ...
    if (s_suppress_copy_in_background.load() &&
        ShouldBackgroundSuppressOperation()) {
...
bool OnUpdateBufferRegion(reshade::api::device *device, const void *data,
                          reshade::api::resource resource, uint64_t offset,
                          uint64_t size) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_UPDATE_BUFFER_REGION].fetch_add(
        1);
    g_swapchain_event_total_count.fetch_add(1);
...
bool OnUpdateBufferRegionCommand(reshade::api::command_list *cmd_list,
                                 const void *data, reshade::api::resource dest,
                                 uint64_t dest_offset, uint64_t size) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_UPDATE_BUFFER_REGION_COMMAND]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
...
bool OnBindResource(reshade::api::command_list *cmd_list,
                    reshade::api::shader_stage stages,
                    reshade::api::descriptor_table table, uint32_t binding,
                    reshade::api::resource_view value) {
    // Increment event counter
...
bool OnMapResource(reshade::api::device *device,
                   reshade::api::resource resource, uint32_t subresource,
    ...
    if (s_suppress_memory_ops_in_background.load() &&
        ShouldBackgroundSuppressOperation()) {
...
void OnUnmapResource(reshade::api::device *device,
                     reshade::api::resource resource, uint32_t subresource) {
...
bool OnCopyBufferRegion(reshade::api::command_list *cmd_list,
                        reshade::api::resource source, uint64_t source_offset,
                        reshade::api::resource dest, uint64_t dest_offset,
                        uint64_t size) {
    // Increment event counter
...
// frame-specific)
bool OnCopyBufferToTexture(reshade::api::command_list *cmd_list,
                           reshade::api::resource source,
                           uint64_t source_offset, uint32_t row_length,
                           uint32_t slice_height, reshade::api::resource dest,
                           uint32_t dest_subresource,
                           const reshade::api::subresource_box *dest_box) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_COPY_BUFFER_TO_TEXTURE]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
...
bool OnCopyTextureToBuffer(reshade::api::command_list *cmd_list,
                           reshade::api::resource source,
                           uint32_t source_subresource,
                           const reshade::api::subresource_box *source_box,
                           reshade::api::resource dest, uint64_t dest_offset,
                           uint32_t row_length, uint32_t slice_height) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_COPY_TEXTURE_TO_BUFFER]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
...
bool OnCopyTextureRegion(reshade::api::command_list *cmd_list,
                         reshade::api::resource source,
                         uint32_t source_subresource,
                         const reshade::api::subresource_box *source_box,
    ...
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_COPY_TEXTURE_REGION].fetch_add(
        1);
    g_swapchain_event_total_count.fetch_add(1);
...
bool OnResolveTextureRegion(reshade::api::command_list *cmd_list,
                            ...
                            const reshade::api::subresource_box *source_box,
                            reshade::api::resource dest,
                            uint32_t dest_subresource, uint32_t dest_x,
                            uint32_t dest_y, uint32_t dest_z,
                            reshade::api::format format) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_RESOLVE_TEXTURE_REGION]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
...
bool OnClearRenderTargetView(reshade::api::command_list *cmd_list,
                             reshade::api::resource_view rtv,
                             const float color[4], uint32_t rect_count,
                             const reshade::api::rect *rects) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_CLEAR_RENDER_TARGET_VIEW]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);

    // Power saving: skip render target clearing in background
    if (s_suppress_clear_ops_in_background.load() &&
        ShouldBackgroundSuppressOperation()) {
...
bool OnClearDepthStencilView(reshade::api::command_list *cmd_list,
                             reshade::api::resource_view dsv,
                             const float *depth, const uint8_t *stencil,
                             uint32_t rect_count,
                             const reshade::api::rect *rects) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_CLEAR_DEPTH_STENCIL_VIEW]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
...
// Power saving for unordered access view clearing with uint values
// (frame-specific)
bool OnClearUnorderedAccessViewUint(reshade::api::command_list *cmd_list,
                                    reshade::api::resource_view uav,
                                    const uint32_t values[4],
                                    uint32_t rect_count,
                                    const reshade::api::rect *rects) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_CLEAR_UNORDERED_ACCESS_VIEW_UINT]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
...
// Power saving for unordered access view clearing with float values
// (frame-specific)
bool OnClearUnorderedAccessViewFloat(reshade::api::command_list *cmd_list,
                                     reshade::api::resource_view uav,
                                     const float values[4], uint32_t rect_count,
                                     const reshade::api::rect *rects) {
    // Increment event counter
    g_swapchain_event_counters
...
bool OnGenerateMipmaps(reshade::api::command_list *cmd_list,
    ...
    if (s_suppress_mipmap_gen_in_background.load() &&
        ShouldBackgroundSuppressOperation()) {
...
bool OnBlit(reshade::api::command_list *cmd_list, reshade::api::resource source,
            uint32_t source_subresource,
            const reshade::api::subresource_box *source_box,
            reshade::api::resource dest, uint32_t dest_subresource,
            const reshade::api::subresource_box *dest_box,
    ...
    if (s_suppress_blit_ops_in_background.load() &&
        ShouldBackgroundSuppressOperation()) {
...
bool OnBeginQuery(reshade::api::command_list *cmd_list,
                  reshade::api::query_heap heap, reshade::api::query_type type,
                  uint32_t index) {
    // Increment event counter
    ...
    if (s_suppress_query_ops_in_background.load() &&
        ShouldBackgroundSuppressOperation()) {
...
bool OnEndQuery(reshade::api::command_list *cmd_list,
                reshade::api::query_heap heap, reshade::api::query_type type,
                uint32_t index) {
    // Increment event counter
...
bool OnResolveQueryData(reshade::api::command_list *cmd_list,
                        reshade::api::query_heap heap,
                        reshade::api::query_type type, uint32_t first,
                        uint32_t count, reshade::api::resource dest,
...
bool OnDrawOrDispatchIndirect(reshade::api::command_list *cmd_list,
                              reshade::api::indirect_command type,
                              reshade::api::resource buffer, uint64_t offset,
                              uint32_t draw_count, uint32_t stride) {
    // Increment event counter
    g_swapchain_event_counters[SWAPCHAIN_EVENT_DRAW_OR_DISPATCH_INDIRECT]
        .fetch_add(1);
    g_swapchain_event_total_count.fetch_add(1);
