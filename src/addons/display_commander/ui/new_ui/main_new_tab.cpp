...
void InitMainNewTab() {
  ...
  if (!settings_loaded_once) {
    // Ensure developer settings (including continuous monitoring) are loaded so
    // UI reflects saved state
    settings::g_developerTabSettings.LoadAll();
    settings::g_mainTabSettings.LoadSettings();
    s_window_mode = static_cast<WindowMode>(
        settings::g_mainTabSettings.window_mode.GetValue());
    s_aspect_index = static_cast<AspectRatioType>(
        settings::g_mainTabSettings.aspect_index.GetValue());
    s_target_monitor_index.store(
        settings::g_mainTabSettings.target_monitor_index.GetValue());
    s_window_alignment = static_cast<WindowAlignment>(
        settings::g_mainTabSettings.alignment.GetValue());
    ...
    s_mute_in_background.store(
        settings::g_mainTabSettings.mute_in_background.GetValue());
    s_mute_in_background_if_other_audio.store(
        settings::g_mainTabSettings.mute_in_background_if_other_audio
            .GetValue());
    s_no_present_in_background.store(
        settings::g_mainTabSettings.no_present_in_background.GetValue());
    ...
    // FPS limiter mode
    s_fps_limiter_mode.store(static_cast<FpsLimiterMode>(
        settings::g_mainTabSettings.fps_limiter_mode.GetValue()));
    // FPS limiter injection timing
    s_fps_limiter_injection.store(
        settings::g_mainTabSettings.fps_limiter_injection.GetValue());
    // Scanline offset and VBlank Sync Divisor are now automatically synced via
    // IntSettingRef
...
void DrawMainNewTab() {
  ...
  ImGui::TextColored(ImVec4(0.8f, 1.0f, 0.8f, 1.0f),
                     "=== Display Commander ===");
  ImGui::TextColored(ImVec4(0.9f, 0.9f, 0.9f, 1.0f), "Version: %s",
                     DISPLAY_COMMANDER_VERSION_STRING);
  ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "Build: %s %s",
  ...
  // Input Blocking (Background) Section
  {
    ...
    if (ImGui::Checkbox("Block Input in Background",
                        &block_any_in_background)) {
      settings::g_mainTabSettings.block_input_in_background.SetValue(
          block_any_in_background);
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Blocks mouse, keyboard, and cursor warping while the "
    ...
    if (ImGui::Checkbox(
            "Block input without Reshade (Uses Windows message hooks)",
            &block_without_reshade)) {
      settings::g_mainTabSettings.block_input_without_reshade.SetValue(
          block_without_reshade);
      // No need to call update function - the message hooks check the setting
      // directly
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Uses Windows message hooks to block input independently of "
          "Reshade's system. Required for future gamepad remapping features.");
    }
  }
  ...
  // Screensaver Settings Section
  {
    ...
    const char *screensaver_modes[] = {"No Change", "Disable in Foreground",
                                       "Always Disable"};

    if (ImGui::Combo("Screensaver Mode", &screensaver_mode, screensaver_modes,
                     3)) {
      settings::g_mainTabSettings.screensaver_mode.SetValue(screensaver_mode);
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Controls how the screensaver behaves while the game is running:\n"
          "No Change: Game controls screensaver normally\n"
          "Disable in Foreground: Disable screensaver only when game window is "
          "focused\n"
          "Always Disable: Always disable screensaver while game is running");
    }
  }
...
void DrawQuickResolutionChanger() {
  ...
  // Quick-set buttons based on current monitor refresh rate
  {
    ...
    if (y > 0) {
      ...
      {
        bool selected =
            (std::fabs(settings::g_mainTabSettings.fps_limit.GetValue() -
                       0.0f) <= selected_epsilon);
        if (selected) {
          ...
          if (candidate_rounded >= 30) {
            ...
            std::string label = std::to_string(candidate_rounded);
            {
              ...
              if (ImGui::IsItemHovered()) {
                ...
                tooltip_oss << "FPS = " << refresh_hz << " ÷ " << x << " = "
                            << candidate_precise << " FPS\n\n";
                tooltip_oss
                    << "Creates a smooth frame rate that divides evenly\n";
        ...
        ImGui::SameLine();
      {
        ...
        bool selected =
            (std::fabs(settings::g_mainTabSettings.fps_limit.GetValue() -
                       precise_target) <= selected_epsilon);
        ...
        if (ImGui::Button("Gsync Cap")) {
          ...
          float target_fps =
              static_cast<float>(precise_target < 1.0 ? 1.0 : precise_target);
          settings::g_mainTabSettings.fps_limit.SetValue(target_fps);
        }
        if (selected) {
          ImGui::PopStyleColor(3);
        ...
        if (ImGui::IsItemHovered()) {
          ...
          tooltip_oss.setf(std::ios::fixed);
          tooltip_oss << std::setprecision(3);
          tooltip_oss << "Gsync Cap: FPS = " << refresh_hz << " - ("
                      << refresh_hz << "² / 3600)\n";
          tooltip_oss << "= " << refresh_hz << " - "
                      << (refresh_hz * refresh_hz / 3600.0) << " = "
                      << gsync_target << " FPS\n\n";
          tooltip_oss
              << "Creates a ~0.3ms frame time buffer to optimize latency\n";
          tooltip_oss << "and prevent tearing, similar to NVIDIA Reflex Low "
                         "Latency Mode.";
          ImGui::SetTooltip("%s", tooltip_oss.str().c_str());
        }
      }
...
void DrawDisplaySettings() {
                     ...
                     "=== Display Settings ===");
  {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Choose which monitor to apply size/pos to. 'Auto' "
                        "uses the current window monitor.");
    }
    ...
    std::string current_display =
        settings::g_mainTabSettings.target_display.GetValue();
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Current display device ID for the game window. "
                        "Updated automatically from display cache.");
    }
  }

  // Window Mode dropdown (with persistent setting)
  if (ComboSettingWrapper(settings::g_mainTabSettings.window_mode,
                          "Window Mode")) {
    WindowMode old_mode = s_window_mode.load();
    s_window_mode = static_cast<WindowMode>(
        settings::g_mainTabSettings.window_mode.GetValue());

    // Don't apply changes immediately - let the normal window management system
    // handle it This prevents crashes when changing modes during gameplay
  ...
  ImGui::SameLine();
  if (CheckboxSetting(settings::g_developerTabSettings.continuous_monitoring,
                      "Auto-apply")) {
    s_continuous_monitoring_enabled.store(
        settings::g_developerTabSettings.continuous_monitoring.GetValue());
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Auto-apply window mode changes and continuously keep "
                      "window size/position in sync.");
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Choose the window mode: Borderless Windowed with aspect "
                      "ratio or Borderless Fullscreen.");
  }

  // Aspect Ratio dropdown (only shown in Aspect Ratio mode)
  if (s_window_mode.load() == WindowMode::kAspectRatio) {
    if (ComboSettingWrapper(settings::g_mainTabSettings.aspect_index,
                            "Aspect Ratio")) {
      s_aspect_index = static_cast<AspectRatioType>(
          settings::g_mainTabSettings.aspect_index.GetValue());
  ...
  if (s_window_mode.load() == WindowMode::kAspectRatio) {
    if (ComboSettingWrapper(settings::g_mainTabSettings.alignment,
                            "Alignment")) {
      s_window_alignment = static_cast<WindowAlignment>(
          settings::g_mainTabSettings.alignment.GetValue());
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Choose how to align the window when repositioning is needed. "
  ...
  if (s_window_mode.load() == WindowMode::kAspectRatio) {
    if (CheckboxSetting(settings::g_mainTabSettings.background_feature,
                        "Background Black Curtain")) {
      LogInfo("Background black curtain setting changed");
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Creates a black background behind the game window "
                        "when it doesn't cover the full screen.");
    }
  }
  ...
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "Apply the current window size and position settings immediately.");
  }
  ...
  // FPS Limiter Mode
  {
    if (ComboSettingWrapper(settings::g_mainTabSettings.fps_limiter_mode,
                            "FPS Limiter Mode")) {
      s_fps_limiter_mode.store(static_cast<FpsLimiterMode>(
          settings::g_mainTabSettings.fps_limiter_mode.GetValue()));
      ...
      if (mode == FpsLimiterMode::kNone) {
        ...
        LogInfo(
            "FPS Limiter: VBlank Scanline Sync for VSYNC-OFF or without VRR");
      }
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Choose limiter: Custom sleep/spin or Latent Sync pacing");
    }
  }

  // FPS Limiter Injection Timing
  {
    int current_injection =
        settings::g_mainTabSettings.fps_limiter_injection.GetValue();
    int temp_injection = current_injection;
    if (ImGui::SliderInt("FPS Limiter Injection", &temp_injection, 0, 2,
                         "%d")) {
      settings::g_mainTabSettings.fps_limiter_injection.SetValue(
          temp_injection);
    ...
    const char *injection_labels[] = {"OnPresentFlags (Recommended)",
                                      "OnPresentUpdateBefore2",
                                      "OnPresentUpdateBefore"};
    if (temp_injection >= 0 && temp_injection < 3) {
      ImGui::TextColored(ImVec4(0.8f, 1.0f, 0.8f, 1.0f), "Current: %s",
                         injection_labels[temp_injection]);
    }
  }
  ...
  // Present Pacing Delay slider (persisted)
  {
    ...
    ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f),
                       "Adds a small delay after present to smooth frame "
                       "pacing and reduce stuttering");

    float current_delay =
        settings::g_mainTabSettings.present_pacing_delay_percentage.GetValue();
    if (SliderFloatSetting(
            settings::g_mainTabSettings.present_pacing_delay_percentage,
            "Present Pacing Delay (manual fine-tuning is needed for now)",
            "%.1f%%")) {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          ...
          "• Higher values provide lower latency but slightly less consistent "
          "timing.\n"
  ...
  if (s_fps_limiter_mode.load() == FpsLimiterMode::kLatentSync) {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Scanline offset for latent sync (-1000 to 1000). This defines the "
          "offset from the threshold where frame pacing is active.");
    }

    // VBlank Sync Divisor (only visible if latent sync mode is selected)
    int current_divisor =
        settings::g_mainTabSettings.vblank_sync_divisor.GetValue();
    ...
    if (ImGui::IsItemHovered()) {
      ...
      std::ostringstream tooltip_oss;
      tooltip_oss << "VBlank Sync Divisor (0-8). Controls frame pacing similar "
                     "to VSync divisors:\n\n";
      ...
      tooltip_oss << "\n0 = Disabled, higher values reduce effective frame "
                     "rate for smoother frame pacing.";
      ImGui::SetTooltip("%s", tooltip_oss.str().c_str());
    }

    // VBlank Monitor Status (only visible if latent sync is enabled and FPS
    // limit > 0)
    if (s_fps_limiter_mode.load() == FpsLimiterMode::kLatentSync) {
      if (dxgi::latent_sync::g_latentSyncManager) {
        auto &latent =
            dxgi::latent_sync::g_latentSyncManager->GetLatentLimiter();
        if (latent.IsVBlankMonitoringActive()) {
          ImGui::Spacing();
          ImGui::TextColored(ImVec4(0.0f, 1.0f, 0.0f, 1.0f),
                             "✁EVBlank Monitor: ACTIVE");
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("VBlank monitoring thread is running and "
                              "collecting scanline data for frame pacing.");
          }

          ImGui::TextColored(
              ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "  refresh time: %.3fms",
              1.0 * dxgi::fps_limiter::ns_per_refresh.load() / utils::NS_TO_MS);
          ImGui::SameLine();
          ImGui::TextColored(
              ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "  total_height: %llu",
              dxgi::fps_limiter::g_latent_sync_total_height.load());
          ImGui::SameLine();
          ImGui::TextColored(
              ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "  active_height: %llu",
              dxgi::fps_limiter::g_latent_sync_active_height.load());
        } else {
          ImGui::Spacing();
          ImGui::TextColored(ImVec4(1.0f, 0.5f, 0.0f, 1.0f),
                             "⚠ VBlank Monitor: STARTING...");
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip("VBlank monitoring is enabled but the monitoring "
  ...
  // FPS Limit slider (persisted)
  {
    ...
    if (SliderFloatSetting(settings::g_mainTabSettings.fps_limit, "FPS Limit",
                           fmt)) {
    }
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Set FPS limit for the game (0 = no limit). Now uses the "
                      "new Custom FPS Limiter system.");
  }

  // No Render in Background checkbox
  {
    bool no_render_in_bg =
        settings::g_mainTabSettings.no_render_in_background.GetValue();
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Skip rendering draw calls when the game window is not "
                        "in the foreground. This can save GPU power and reduce "
  ...
  // No Present in Background checkbox
  {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Skip ReShade's on_present processing when the game "
                        "window is not in the foreground. This can save GPU "
                        "power and reduce background processing.");
    }
  }
  ...
  // VSync & Tearing controls
  {
    ...
    if (ImGui::Checkbox("Prevent Tearing", &prevent_t)) {
      ...
      LogInfo(prevent_t
                  ? "Prevent Tearing enabled (tearing flags will be cleared)"
                  : "Prevent Tearing disabled");
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Prevents tearing by clearing DXGI tearing flags and "
                        "preferring sync.");
    }
    ...
    if (s_restart_needed_vsync_tearing.load()) {
      ...
      ImGui::TextColored(
          ...
          "Game restart required to apply VSync/tearing changes.");
    }
  }
  ...
  {
    float current_bg =
        settings::g_mainTabSettings.fps_limit_background.GetValue();
    const char *fmt_bg = (current_bg > 0.0f) ? "%.0f FPS" : "No Limit";
    if (SliderFloatSetting(settings::g_mainTabSettings.fps_limit_background,
                           "Background FPS Limit", fmt_bg)) {
    }
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("FPS cap when the game window is not in the foreground. "
                      "Now uses the new Custom FPS Limiter system.");
  }
}
...
void DrawAudioSettings() {
  ...
  ImGui::SameLine();
  if (CheckboxSetting(settings::g_mainTabSettings.audio_volume_auto_apply,
                      "Auto-apply##audio_volume")) {
  ...
  if (ImGui::Checkbox("Mute In Background", &mute_in_bg)) {
    ...
    settings::g_mainTabSettings.mute_in_background_if_other_audio.SetValue(
        false);
    ...
    if (!s_audio_mute.load()) {
      ...
      bool want_mute =
          (mute_in_bg && hwnd != nullptr && GetForegroundWindow() != hwnd);
  ...
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "Mute the game's audio when it is not the foreground window.");
  }
  if (s_audio_mute.load()) {
    ImGui::EndDisabled();
  }
  ...
  if (ImGui::Checkbox("Mute In Background (only if other app has audio)",
                      &mute_in_bg_if_other)) {
    settings::g_mainTabSettings.mute_in_background_if_other_audio.SetValue(
        mute_in_bg_if_other);
    ...
    if (!s_audio_mute.load()) {
      ...
      bool want_mute =
          (mute_in_bg_if_other && is_background && ::IsOtherAppPlayingAudio());
      if (::SetMuteForCurrentProcess(want_mute)) {
        ::g_muted_applied.store(want_mute);
        std::ostringstream oss;
        oss << "Background mute (if other audio) "
            << (mute_in_bg_if_other ? "enabled" : "disabled");
        LogInfo(oss.str().c_str());
      }
    }
  }
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "Mute only if app is background AND another app outputs audio.");
  }
  if (s_audio_mute.load()) {
    ImGui::EndDisabled();
  }
...
void DrawWindowControls() {
  ...
  if (ImGui::Button("Maximize Window")) {
    std::thread([hwnd, current_monitor_width, current_monitor_height]() {
      ...
      settings::g_mainTabSettings.window_mode.SetValue(
          static_cast<int>(WindowMode::kFullscreen));
...
void DrawMonitorDisplaySettings() {
  ...
  // Resolution Widget
  if (ImGui::CollapsingHeader("Resolution Control",
...
void DrawImportantInfo() {
  ...
  ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.8f, 1.0f),
                     "=== Important Information ===");

  // Test Overlay Control
  {
    bool show_test_overlay =
        settings::g_mainTabSettings.show_test_overlay.GetValue();
    if (ImGui::Checkbox("Show Test Overlay (reshade_overlay demo)",
                        &show_test_overlay)) {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Shows a test widget using the reshade_overlay event "
                        "to demonstrate the difference between "
                        "register_overlay and reshade_overlay approaches.");
    }
  }
  ...
  // FPS Counter with 1% Low and 0.1% Low over past 60s (computed in background)
  {
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Reset FPS/frametime statistics. Metrics are computed since reset.");
    }
  }
  std::ostringstream oss;
  ...
  oss << "Render Submit Duration: " << std::fixed << std::setprecision(3)
      << (1.0 * ::g_render_submit_duration_ns.load() / utils::NS_TO_MS)
      << " ms";
  ImGui::TextUnformatted(oss.str().c_str());
  ImGui::SameLine();
  ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "(smoothed)");
  ...
  oss << "Reshade Overhead Duration: " << std::fixed << std::setprecision(3)
      << ((1.0 * ::g_reshade_overhead_duration_ns.load() -
           ::fps_sleep_before_on_present_ns.load() -
  ...
  oss.str("");
  oss.clear();
  oss << "FPS Limiter Sleep Duration (before onPresent): " << std::fixed
      << std::setprecision(3)
      << (1.0 * ::fps_sleep_before_on_present_ns.load() / utils::NS_TO_MS)
      << " ms";
  ImGui::TextUnformatted(oss.str().c_str());
  ImGui::SameLine();
  ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "(smoothed)");
  ...
  oss.str("");
  oss.clear();
  oss << "FPS Limiter Sleep Duration (after onPresent): " << std::fixed
      << std::setprecision(3)
      << (1.0 * ::fps_sleep_after_on_present_ns.load() / utils::NS_TO_MS)
      << " ms";
  ImGui::TextUnformatted(oss.str().c_str());
  ImGui::SameLine();
  ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), "(smoothed)");
  ...
  if (current_fps > 0.0f) {
    ...
    float sleep_duration_ms =
        static_cast<float>(::fps_sleep_after_on_present_ns.load()) /
        utils::NS_TO_MS;
    float latency_ms = frame_time_ms - sleep_duration_ms;

    static double sim_start_to_present_latency_ms = 0.0;
    sim_start_to_present_latency_ms =
        (sim_start_to_present_latency_ms * 0.99 + latency_ms * 0.01);
    oss << "Sim Start to Present Latency: " << std::fixed
        << std::setprecision(3) << sim_start_to_present_latency_ms << " ms";
    ImGui::TextUnformatted(oss.str().c_str());
    ImGui::SameLine();
    ImGui::TextColored(ImVec4(0.8f, 1.0f, 0.8f, 1.0f),
                       "(frame_time - sleep_duration)");
  }
...
void DrawAdhdMultiMonitorControls() {
  ...
  if (ImGui::Checkbox("ADHD Multi-Monitor Mode", &adhdEnabled)) {
    settings::g_mainTabSettings.adhd_multi_monitor_enabled.SetValue(
        adhdEnabled);
  ...
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Covers secondary monitors with a black window to reduce "
                      "distractions while playing this game.");
  }

  // Focus disengagement is always enabled (no UI control needed)
  ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f),
                     "• Automatically disengages on Alt-Tab");
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "ADHD mode will automatically disengage whenever you Alt-Tab, "
        "regardless of which monitor the new application is on.");
  }

  // Additional information
  ImGui::Spacing();
  ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.8f, 1.0f),
                     "This feature helps reduce distractions by covering "
                     "secondary monitors with a black background.");
  if (ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "Similar to Special-K's ADHD Multi-Monitor Mode.\nThe black background "
        "window will automatically position itself to cover all monitors "
        "except the one where your game is running.");
  }
}
