...
bool DLSSPresetDetector::IsAvailable() const {
    return initialized && current_preset.valid;
}

const DLSSPresetDetector::PresetInfo& DLSSPresetDetector::GetPreset() const {
...
const std::string& DLSSPresetDetector::GetLastError() const {
    return last_error;
}
...
std::string DLSSPresetDetector::GetPresetNameFromValue(unsigned int preset_value) const {
    switch (preset_value) {
        ...
        case NVSDK_NGX_DLSS_Hint_Render_Preset_G:       return "G";
