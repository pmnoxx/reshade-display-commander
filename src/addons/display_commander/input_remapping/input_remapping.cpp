...
namespace display_commander::input_remapping
{
    ...
    void InputRemapper::add_button_remap(const ButtonRemap& remap)
    {
        ...
        LogInfo("InputRemapper::add_button_remap() - Added remap for button 0x%04X to key %s",
                remap.gamepad_button, remap.keyboard_name.c_str());
    }
    ...
    void InputRemapper::set_default_input_method(KeyboardInputMethod method)
    {
        ...
        LogInfo("InputRemapper::set_default_input_method() - Set to %s",
                get_keyboard_input_method_name(method).c_str());
    }
    ...
    void InputRemapper::load_settings()
    {
        ...
        if (reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", "Count", remapping_count)) {
            ...
            for (int i = 0; i < remapping_count; ++i) {
                ...
                if (reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "GamepadButton").c_str(), gamepad_button) &&
                    reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "KeyboardVk").c_str(), keyboard_vk) &&
                    reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "InputMethod").c_str(), input_method) &&
                    reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "Enabled").c_str(), enabled) &&
                    reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "HoldMode").c_str(), hold_mode) &&
                    reshade::get_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "KeyboardName").c_str(), keyboard_name, &keyboard_name_size)) {
            ...
            add_button_remap(ButtonRemap(XINPUT_GAMEPAD_B, VK_ESCAPE, "Escape", true, KeyboardInputMethod::SendInput, false));
    ...
    void InputRemapper::save_settings()
    {
        ...
        reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", "DefaultMethod", static_cast<int>(_default_input_method));

        // Save remappings count
        reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", "Count", static_cast<int>(_remappings.size()));
        ...
        for (size_t i = 0; i < _remappings.size(); ++i) {
            ...
            reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "GamepadButton").c_str(), static_cast<int>(remap.gamepad_button));
            reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "KeyboardVk").c_str(), remap.keyboard_vk);
            reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "InputMethod").c_str(), static_cast<int>(remap.input_method));
            reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "Enabled").c_str(), remap.enabled);
            reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "HoldMode").c_str(), remap.hold_mode);
            reshade::set_config_value(nullptr, "DisplayCommander.InputRemapping", (key_prefix + "KeyboardName").c_str(), remap.keyboard_name.c_str());
        }
    ...
    HWND InputRemapper::get_active_window() const
    {
    ...
    void InputRemapper::handle_button_release(WORD gamepad_button, DWORD user_index)
    {
        ...
        if (success) {
            LogInfo("InputRemapper::handle_button_release() - Released %s (Controller %lu)",
    ...
    {
        InputRemapper::get_instance().initialize();
    }

    void cleanup_input_remapping()
    {
        InputRemapper::get_instance().cleanup();
    }
