...
static bool ChangeColorSpace(reshade::api::swapchain *swapchain, reshade::api::color_space color_space) {
    if (IsDXGI(swapchain)) {
        ...
        case reshade::api::color_space::srgb_nonlinear:
            dx_color_space = DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709;
            break;
        case reshade::api::color_space::extended_srgb_linear:
            dx_color_space = DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709;
            break;
        case reshade::api::color_space::hdr10_st2084:
            dx_color_space = DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020;
            break;
        case reshade::api::color_space::hdr10_hlg:
            dx_color_space = DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P2020;
            break;
        default:
            return false;
        }
        ...
        if (FAILED(hr)) {
            ...
            std::stringstream s;
            s << "utils::swapchain::ChangeColorSpace(Failed to set DirectX color space, hr = 0x" << std::hex << hr
              << std::dec << ")";
...
static void OnDestroyDevice(reshade::api::device *device) { device->destroy_private_data<DeviceData>(); }
