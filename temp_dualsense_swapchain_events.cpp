...
void OnPresentUpdateAfter(reshade::api::command_queue* /*queue*/, reshade::api::swapchain* swapchain) {
    ...
    if (previous_render_thread_id != current_thread_id && previous_render_thread_id != 0) {
        ...
        count++;
        if (count <= 10) {
            LogDebug("[TID:" + std::to_string(current_thread_id) + "] Render thread changed from "
                     + std::to_string(previous_render_thread_id) + " to " + std::to_string(current_thread_id));
        }
    }
    ...
    double g_present_duration_new_ns =
        (now_ns - g_present_start_time_ns.load());  // Convert QPC ticks to seconds (QPC frequency is typically 10MHz)
    double alpha = 64;
    ...
    if (swapchain != nullptr) {
        ...
        if (present_after_counter >= 240) {  // Refresh every 240 presents (about 4 seconds at 60fps)
            ...
            DxgiBypassMode last = g_comp_last_logged.load();
            if (mode != last) {
                g_comp_last_logged.store(mode);
                std::ostringstream oss;
                oss << "DXGI Composition State (OnPresentAfter): " << DxgiBypassModeToString(mode) << " (" << static_cast<int>(mode) << ")";
                LogInfo(oss.str().c_str());
            }
...
void OnPresentUpdateBefore(reshade::api::command_queue* /*queue*/, reshade::api::swapchain* swapchain,
    ...
    if (g_flush_before_present.load()) {
        flush_command_queue();  // Flush command queue before addons start processing to reduce rendering latency caused
                                // by reshade
    }
...
void OnPresentFlags(uint32_t* present_flags, reshade::api::swapchain* swapchain) {
    ...
    if (s_prevent_tearing.load() && *present_flags & DXGI_PRESENT_ALLOW_TEARING) {
        *present_flags &= ~DXGI_PRESENT_ALLOW_TEARING;
        ...
        std::ostringstream oss;
        oss << "Present flags callback: Stripped DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING, new flags: 0x" << std::hex
            << *present_flags;
        LogInfo(oss.str().c_str());
    }
